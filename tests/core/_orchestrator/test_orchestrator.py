...
# ################################  UTIL METHODS    ##################################
def _assert_available_workers(dispatcher: _StandaloneJobDispatcher, expected_workers: int):
    assert_true_after_time(lambda: dispatcher._nb_available_workers == expected_workers)


def _assert_blocked_jobs_count(expected_count: int):
    assert_true_after_time(lambda: len(_Orchestrator.blocked_jobs) == expected_count)
