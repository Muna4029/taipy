...
def test_need_to_run_no_output():
    hello_cfg = Config.configure_data_node("hello_validity_period", default_data="Hello ")
    world_cfg = Config.configure_data_node("world", default_data="world !")
    task_cfg = Config.configure_task(
        "name_validity_period", input=[hello_cfg, world_cfg], function=nothing, output=[]
    )
...
def test_need_to_run_skippable_task_no_validity_period_on_output():
    hello_cfg = Config.configure_data_node("hello_validity_period", default_data="Hello ")
    output_cfg = Config.configure_data_node("output_validity_period")
    task_cfg = Config.configure_task(
...
def test_need_to_run_skippable_task_with_validity_period_on_output():
    hello_cfg = Config.configure_data_node("hello_validity_period", default_data="Hello ")
    hello_world_cfg = Config.configure_data_node("output_validity_period", validity_period=timedelta(days=1))
    task_cfg = Config.configure_task(
        "name_validity_period", nothing, [hello_cfg], [hello_world_cfg], skippable=True
    )
    task = _create_task_from_config(task_cfg)
    dispatcher = _JobDispatcher(_OrchestratorFactory._build_orchestrator())
...
def test_need_to_run_skippable_task_but_input_edited_after_output():
    ...
    hello_world_cfg = Config.configure_data_node("output_validity_period")
    task_cfg = Config.configure_task(
        "name_validity_period", nothing, [hello_cfg], [hello_world_cfg], skippable=True
    )
    task = _create_task_from_config(task_cfg)
