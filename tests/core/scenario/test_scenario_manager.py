...
def test_is_deletable():
    assert len(_ScenarioManager._get_all()) == 0
    scenario_config = Config.configure_scenario("sc", None, None, Frequency.DAILY)
    creation_date = datetime(2021, 1, 1, 12, 0, 0)
...
def test_tags():
    cycle_1 = _CycleManager._create(Frequency.DAILY, name="today", creation_date=datetime(2021, 1, 1, 12, 0, 0))
    cycle_2 = _CycleManager._create(
        Frequency.DAILY,
        name="tomorrow",
        creation_date=datetime(2021, 1, 1, 12, 0, 0) + timedelta(days=1),
    )
...
def test_duplicate_scenario():
    ...
    with mock.patch.object(_ScenarioManager, "_can_duplicate", return_value=ReasonCollection()) as mock_can:
        with mock.patch.object(_ScenarioDuplicator, "duplicate") as mock_duplicate:
            ...
            new_date = datetime(2021, 1, 1, 12, 0, 0)
            new_name = "new_name"
