...
class ExcelDataNode(DataNode, _FileDataNodeMixin, _TabularDataNodeMixin):
    ...
    def _read_as(self, path: str):
        try:
            ...
            raw_sheet_names = properties.get(self.__SHEET_NAME_PROPERTY) or []
            if isinstance(raw_sheet_names, (list, set, tuple)):
                user_provided_sheet_names = list(raw_sheet_names)
            else:
                user_provided_sheet_names = [raw_sheet_names]
